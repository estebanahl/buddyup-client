...
const Chat = () => {
  ...
  useEffect(()=>{
    ...
    const fetchUserData = async () => {
      try {
        const endpoint = 'https://buddyup.azurewebsites.net/api/account/get-by-id-or-email';
        const config = {
          headers: {
   ...
   const saveUserData=(res)=> {
    setDataUser({name:res.pname, img:res.images[0].image_url,tags:res.tags})
   }
  ...
  >
	<form  onSubmit={handleSendMessage}>
        <div className='flex flex-col justify-center items-center'>
          ...
          <div className='flex justify-center items-center flex-wrap flex-row gap-2'>{dataUser.tags?.map((item,index)=>(
            <div className='text-black  px-8 items-start gap-1  rounded-xl border border-gray-700 bg-white' key={index}>
            <p>{item.name}</p>
          </div>
          ))}
          </div>
